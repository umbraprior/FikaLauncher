...
public static class TermsService
{
    public static string GetProcessedTerms(bool isDark, bool isLauncherTerms = true)
    {
        try
        {
            ...
            Console.WriteLine($"Attempting to load terms from: {resourcePath}");

            using var stream = assembly.GetManifestResourceStream(resourcePath);
            if (stream == null)
            {
                Console.WriteLine($"Failed to load terms file: {resourcePath}");
                ...
                throw new FileNotFoundException($"Terms file not found: {resourcePath}");
            }
